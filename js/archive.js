...
function row(w, producerById) {
  ...
  const head = el(
    'div.arow-head',
    null,
    el('span.a-title', null,
      el('span.a-t', null, titleNodes(t(w.title))),
      /* 부제는 제목 아래 작고 옅게. 없는 항목에는 줄이 생기지 않는다. */
      t(w.subtitle) ? el('span.a-sub', { text: t(w.subtitle) }) : null),
  ...
  /* 자료가 하나도 없으면 아코디언을 만들지 않는다 — 눌러도 열릴 게 없다.
     아카이브 항목의 소개·정보는 아코디언이 아니라 상세 페이지가 담는다
     (커넥티드 시티와 같은 구조). 여기에는 자료 링크와 이어지는 링크만 둔다. */
  if (!mats.length) {
  ...
  /* '자세히 보기'는 본문이 있는 항목에만 붙인다.
     본문이 없으면 상세 페이지에 아코디언만 남아 이 행보다 나을 것이 없다 —
     갈 곳 없는 링크를 두지 않는다. */
  if (t(w.body)) box.append(el('a.a-more', { href: pageUrl('work', w.id), text: '자세히 보기 →' }));
